      ...
      <aside className="academic-sidebar gradebook-sidebar">
        ...
        <nav aria-label="Teacher navigation">
          {navigation.map((item) => (
            <Link
              className={
                item.href === "/teacher/gradebook" ? "is-active" : ""
              }
              href={item.href}
              key={item.label}
            >
              ...
              {item.label}
              {item.label === "Gradebook" && missingCount > 0 ? (
                <b>{missingCount}</b>
              ) : null}
            </Link>
          ))}
        </nav>
        <div className="gradebook-lock-note">
          <span>Grading policy v{workspace.period.policyVersion}</span>
          <strong>
            {workspace.categories
              .map((category) => `${category.weightPercent}%`)
              .join(" / ")}
          </strong>
          <small>Stored with this academic period</small>
        </div>
      </aside>

      <section className="gradebook-main">
        <header className="gradebook-topbar">
          <div>
            <p>Teaching / {workspace.subjectName}</p>
            <h1>Gradebook & reports</h1>
          </div>
          <div className="gradebook-account">
            <span className={`gradebook-data-dot ${dataMode}`} />
            <div>
              <strong>{actor}</strong>
              <small>
                {dataMode === "protected"
